...
`
curl -k https://uat.raad.moh.gov.so/download/install-raad.sh | sudo bash
`
...
1. Open terminal and install `raad-updater` with the follwing command

`
curl -k https://uat.raad.moh.gov.so/download/install-updater | sudo bash
`

2. Update using `raad-update` command

`
sudo /opt/ehr_docker/bin/raad-update
`

3. Finally upgrade using `raad-upgrade` command

`
/opt/ehr_docker/bin/raad-upgrade
`
